...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in ('sender', 'receiver'):
        print("Usage: quick-mode-test.py [sender|receiver] [args...]")
        sys.exit(1)
    ...
    if mode == 'sender':
        target = which('quick-mode-sender-auto.py')
    else:
        target = which('quick-mode-receiver-auto.py')
