...
def send_file(pi, nrf, path, address):
    ...
    o_size = len(data); filename = os.path.basename(path); md5 = md5sum(data)
    compressed = compress_zstd(data, level=ZSTD_LEVEL)
    size = len(compressed)
    compression_used = "zstd"
    header_obj = {'filename': filename, 'o_size': o_size, 'md5': md5, 'size': size, 'compression': compression_used, 'compression_level': ZSTD_LEVEL}
...
def main():
    p = argparse.ArgumentParser(description="NRF24 Auto File Sender (MD5) + zstd compression + optional IRQ pull-up")
