...
int main(int argc, char *argv[])
{
    ...
    /* --- Send 4-byte file size header as first payload --- */
    ...
    if (fsize_long > 0xFFFFFFFFL) {
        fprintf(stderr, "File too large for 32-bit header\n");
        nrf24_close(&dev);
        fclose(f);
        return 1;
    }

    uint32_t file_size = (uint32_t)fsize_long;
    ...
    /* First payload = file size header (little-endian) */
    uint8_t header[PAYLOAD_SIZE];
    memset(header, dev.padding, PAYLOAD_SIZE); /* fill with padding byte */
    memcpy(header, &file_size, sizeof(file_size));

    rc = nrf24_send(&dev, header, PAYLOAD_SIZE);
    if (rc < 0) {
        fprintf(stderr, "send header error %d\n", rc);
        nrf24_close(&dev);
        fclose(f);
        return 1;
    }
    //nrf24_wait_until_sent(&dev);
    printf("First payload sent");
    ...
    while (n > 0) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (n <= 0) break;
            total_bytes += n;
            printf("Total bytes sent:%u (i:%u)", total_bytes, i);
